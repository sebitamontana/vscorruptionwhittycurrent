...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'deprave' | 'lo-fight-b-side':
				dialogue = CoolUtil.coolTextFile(Paths.txt('deprave/depraveDialogue'));
			case 'pressure' | 'overhead-b-side':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pressure/pressureDialogue'));
			case 'infernum' | 'ballistic-b-side':
				dialogue = CoolUtil.coolTextFile(Paths.txt('infernum/infernumDialogue'));
		}
      ...
      case 'alley' | 'ballisticAlley':
        {
          	defaultCamZoom = 0.9;
            curStage = 'whitty';
            wBg = new FlxSprite(-500, -300).loadGraphic(Paths.image('whittyBackC', 'bonusWeek'));

            if (SONG.stage == 'ballisticAlley')
            {
              trace('pogging');
			  wBg.antialiasing = true;
			  var bgTex = Paths.getSparrowAtlas('BallisticBackgroundC', 'bonusWeek');
              ...
              add(nwBg);
              nwBg.alpha = 0;
              wstageFront = new FlxSprite(-650, 600).loadGraphic(Paths.image('whittyFrontC', 'bonusWeek'));
			...
			default:
			{
					defaultCamZoom = 0.9;
					curStage = 'stage';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stagebackC'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('stagefrontC'));
					...
					var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('stagecurtainsC'));
		...
		switch (SONG.player2)
		{
			...
			case 'whittyCrazy':
				camPos.x += 400;
			case 'whittyCrazyCorrupt':
				camPos.x += 400;
			case 'whittyCrazyCorrupt2':
				camPos.x += 400;
			case 'pico':
				camPos.x += 600;
				dad.y += 300;
				...
				if (SONG.stage == 'ballisticAlley')
					{
						wBg.alpha = 0;
						nwBg.alpha = 1;
						funneEffect = new FlxSprite(-600, -200).loadGraphic(Paths.image('thefunnyeffectC', 'bonusWeek'));
	...
	function turnToCrazyWhitty()
	{
		...
		iconP2 = new HealthIcon('whittyCrazyCorrupt', false);
		iconP2.y = healthBar.y - (iconP2.height / 2);
		...
		remove(dad);
		remove(gf);
		dad = new Character(100,100,'whittyCrazyCorrupt');
		add(gf);
		add(dad);
	...
	function whittyAnimation(?dialogueBox:DialogueBox, bside:Bool):Void
	{
		...
		switch(curSong.toLowerCase()) // WHITTY ANIMATION CODE LMAOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
		{
			...
			case 'deprave' | 'lo-fight-b-side':
				...
				animation.frames = Paths.getSparrowAtlas('whittyCutsceneC','bonusWeek');
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (SONG.noteStyle)
			{
				...
				case 'normal':
					babyArrow.frames = Paths.getSparrowAtlas('NOTE_assetsC');
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (curSong.toLowerCase().contains('infernum') && storyDifficulty == 2 && songScore >= 160000)
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'Infernum') 
			{
				switch (curStep) //kur aut LOL
				{
					case 2348: //1738, ay, i'm like hey what's up hello
					FlxG.camera.flash(FlxColor.WHITE, 2, false);
					health = 0.30;
					remove(boyfriend);
					boyfriend = new Boyfriend(770, 450, 'bf-infernum');
					add(boyfriend);
					iconP1.animation.play('bf-infernum');
					remove(dad);
					dad = new Character(115, 100, 'whittyCrazyCorrupt2');
					add(dad);
					iconP2.animation.play('whittyCrazyCorrupt2'); //i am so lonely
					case 4264: //penis
					FlxG.camera.flash(FlxColor.WHITE, 2, false);
					remove(boyfriend);
					boyfriend = new Boyfriend(770, 450, 'bf-corrupt');
					add(boyfriend);
					iconP1.animation.play('bf-corrupt');
					remove(dad);
					dad = new Character(115, 100, 'whittyCrazyCorrupt3');
					add(dad);
					iconP2.animation.play('whittyCrazyCorrupt3'); 
				 }
			}
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % 2 == 0 && curSong.toLowerCase() == 'infernum')
			{
